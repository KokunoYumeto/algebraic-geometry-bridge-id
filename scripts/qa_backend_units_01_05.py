#!/usr/bin/env python3
"""Fail-closed replay and closure QA for the cumulative Units 1--5 backend."""
...
EXPORTER = ROOT / "scripts" / "export_backend_units_01_05.py"
BACKEND = ROOT / "backend" / "units-01-05"
MANIFEST = BACKEND / "MANIFEST.json"
RECEIPT = ROOT / "qa" / "UNITS_01_05_BACKEND_QA.json"
...
RIGHTS_PATH = ROOT / "authority" / "RIGHTS-unit-05.csv"
CLOSURE_PATH = ROOT / "authority" / "ASSET_CLOSURE-unit-05.json"
...
NEW_TERM_BINDINGS = {
    "AGT-0036": ("concept.homogeneous-component", "komponen homogen"),
    "AGT-0037": ("concept.homogeneous-polynomial", "polinom homogen"),
    "AGT-0038": ("concept.total-degree", "derajat total"),
    "AGT-0039": ("concept.noether-normalization", "normalisasi Noether"),
    "AGT-0040": ("concept.polynomial-map", "pemetaan polinomial"),
    "AGT-0041": ("concept.affine-linear-equivalence", "ekuivalensi afin-linear"),
    "AGT-0042": ("concept.integral-ring-extension", "perluasan gelanggang integral"),
}
...
manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
require(manifest["through_unit"] == 5, "Manifest is not cumulative through Unit 5")
require(manifest["scope"] == "cumulative Units 1--5", "Manifest scope mismatch")
...
require(digest(BASELINE / "records.jsonl") == BASELINE_RECORDS_SHA256, "Units 1--4 baseline hash changed")
...
require(len(baseline_raw_by_id) == BASELINE_RECORD_COUNT, "Frozen Units 1--4 baseline record count changed")
for stable_id, baseline_line in baseline_raw_by_id.items():
        require(cumulative_raw_by_id.get(stable_id) == baseline_line, f"Units 1--4 record bytes changed: {stable_id}")
...
require(build_receipt["through_unit"] == 5, "Reader build receipt scope mismatch")
...
current_artifacts = {
    record["path"]: record
    for record in combined_records
    if record["entity_class"] == "artifact" and record["stable_id"].startswith("artifact.units0105.")
}
required_artifact_paths = [row["path"] for row in build_receipt["outputs"]] + [
    BUILD_RECEIPT_PATH.relative_to(ROOT).as_posix(),
    "qa/UNITS_01_05_MACHINE_QA.json",
    "qa/UNITS_01_05_VISUAL_QA.json",
    "qa/UNITS_01_05_RESPONSIVE_QA.json",
    "qa/UNIT_05_PROTECTED_SURFACES.json",
]
...
require(authority_manifest["unit_number"] == 5, "Authority manifest unit mismatch")
authority_dir = AUTHORITY_MANIFEST_PATH.parent
for row in authority_manifest["files"]:
    path = authority_dir / row["file"]
    require(path.is_file(), f"Unit 5 authority file absent: {row['file']}")
    require(path.stat().st_size == row["bytes"], f"Unit 5 authority byte mismatch: {row['file']}")
    require(digest(path) == row["sha256"], f"Unit 5 authority hash mismatch: {row['file']}")
...
for source_name, authority_key in (("lecture-05.md", "lecture"), ("worksheet-05.md", "worksheet")):
...
worksheet_path = ROOT / "source" / "id-ID" / "worksheet-05.md"
solutions_path = ROOT / "source" / "id-ID" / "worksheet-05-solutions.md"
exercises = heading_ids(worksheet_path, r"br-ak-2025-2026-w05-ex-\d{2}")
solutions = heading_ids(solutions_path, r"br-ak-2025-2026-w05-sol-\d{2}")
require(len(exercises) == 27, "Unit 5 exercise heading count mismatch")
require(len(solutions) == 4, "Unit 5 solution heading count mismatch")
authority_map = json.loads(UNIT5_MAP_PATH.read_text(encoding="utf-8"))
...
require(source_solution_numbers == mapped_numbers == [3, 15, 19, 20], "Unit 5 solution-map mismatch")
require({f"exercise.{identifier}" for identifier in exercises} <= class_ids["exercise"], "Unit 5 exercise projections missing")
require({f"solution.{identifier}" for identifier in solutions} <= class_ids["solution"], "Unit 5 solution projections missing")
...
for number, authority_row in mapped_rows.items():
    solution_id = f"solution.br-ak-2025-2026-w05-sol-{number:02d}"
    exercise_id = f"exercise.br-ak-2025-2026-w05-ex-{number:02d}"
    ...
    require(binding["map_sha256"] == digest(UNIT5_MAP_PATH), f"Solution map hash absent: {solution_id}")
...
require(len(class_ids["exercise"]) == 134, "Cumulative exercise-family count mismatch")
require(len(class_ids["solution"]) == 28, "Cumulative solution-family count mismatch")

rights_rows = read_csv(RIGHTS_PATH)
closure = json.loads(CLOSURE_PATH.read_text(encoding="utf-8"))
require(len(rights_rows) == 3, "Unit 5 rights-row count mismatch")
require(closure["reader_media_positions"] == 3, "Unit 5 media-position mismatch")
require(closure["rights_sha256"] == digest(RIGHTS_PATH), "Unit 5 rights closure hash mismatch")
...
require(len(unique_local_paths) == closure["unique_local_assets"] == 4, "Unit 5 unique-asset closure mismatch")
require(len(class_ids["asset"]) == 41, "Cumulative primary-asset count mismatch")
...
require(len(class_ids["concept"]) == 42 and len(class_ids["term"]) == 42, "Cumulative terminology-family count mismatch")

correction_rows = read_csv(CORRECTIONS_PATH)
require(len(correction_rows) == 16, "Current correction ledger does not contain sixteen rows")
require(len(class_ids["correction"]) == 16, "Cumulative correction-family count mismatch")
...
result = {
    ...
    "units_01_04_record_bytes_preserved": len(baseline_raw_by_id),
    "parent_closure": True,
    ...
    "authority_binding": {
        "unit_5_manifest_sha256": digest(AUTHORITY_MANIFEST_PATH),
    ...
    "terminology": {
        ...
        "unit_05_rows": sorted(NEW_TERM_BINDINGS),
    ...
    "check_families": [
        ...
        "stable_id_uniqueness_format_and_units0104_byte_preservation",
        ...
        "unit5_revision_authority_manifest_and_file_hash_replay",
        "unit5_exercise_solution_family_and_map_closure",
        "unit5_component_rights_asset_and_hash_closure",
        "unit5_terminology_control_and_concept_term_closure",
        "sixteen_row_correction_ledger_hash_closure",
