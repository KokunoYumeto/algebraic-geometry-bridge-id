#!/usr/bin/env python3
"""Create bounded contact sheets for visual review of all Units 1--27 pages."""
...
def replace_once(text: str, old: str, new: str) -> str:
    ...
    if count != 1:
        raise SystemExit(f"Unit 27 contact-sheet specialization expected one occurrence, found {count}: {old!r}")
...
for old, new in (
    ...
    ('OUTPUT_DIR = ROOT / "tmp" / "pdfs" / "units-01-24-contact"', 'OUTPUT_DIR = ROOT / "tmp" / "pdfs" / "units-01-27-contact"'),
    ('EXPECTED_PAGES = 417', 'EXPECTED_PAGES = 464'),
    ('page-001..page-417', 'page-001..page-464'),
):
