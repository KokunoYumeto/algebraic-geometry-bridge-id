#!/usr/bin/env python3
"""Freeze Unit 14 Commons media and component-rights evidence."""
...
META = ROOT / "authority" / "commons-imageinfo-unit-14.json"
ASSET = ROOT / "authority" / "assets" / "Monkey_Saddle_Surface_Shaded-500.png"
RIGHTS = ROOT / "authority" / "RIGHTS-unit-14.csv"
CLOSURE = ROOT / "authority" / "ASSET_CLOSURE-unit-14.json"
CREDITS = ROOT / "source" / "id-ID" / "media-credits-unit-14.md"
RESOURCE_TITLE = "File:Monkey_Saddle_Surface_(Shaded).png"
CAPTION = "Grafik suatu fungsi global pada ruang afin dua dimensi"
ALT = "Permukaan pelana monyet berwarna tembaga dengan tiga lembah dan tiga punggung"
...
def main() -> int:
    ...
    if manifest.get("unit_number") != UNIT:
        raise RuntimeError("Authority manifest is not Unit 14")
    ...
    if substantive != [RESOURCE_TITLE.removeprefix("File:")]:
        raise RuntimeError(f"Unexpected Unit 14 substantive media closure: {substantive}")
    ...
    if len(media_infos) != 1:
        raise RuntimeError("No unique Commons imageinfo for Unit 14 media")
    info = media_infos[0]
    ...
    if not ASSET.is_file():
        ASSET.write_bytes(fetch(thumb_url, accept="image/png"))
    if ASSET.is_symlink():
        raise RuntimeError("Unit 14 media asset must be a regular file")
    with Image.open(ASSET) as image:
        image.verify()
    with Image.open(ASSET) as image:
        width, height = int(image.width), int(image.height)
    if (width, height) != (int(info["thumbwidth"]), int(info["thumbheight"])):
        raise RuntimeError("Local thumbnail dimensions disagree with Commons")
    ...
    row = {
        ...
        "selected_url": thumb_url,
        "selected_form": "official Commons 500px thumbnail",
    ...
    CREDITS.write_text(
        "\n".join(
            [
                "# Kredit media Unit 14 {#agc-media-credits-unit-14}",
                "",
    ...
    print(json.dumps({"status": "PASS", "unit": 14, "media_positions": 1, "binary_surfaces": 1, "pdf_witnesses": 2, "rights_sha256": digest(RIGHTS), "closure_sha256": digest(CLOSURE)}, ensure_ascii=False))
    return 0
