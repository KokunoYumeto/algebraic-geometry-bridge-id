#!/usr/bin/env python3
"""Publish the verified Unit 12 checkpoint in the existing Zenodo concept.
...
zenodo.ROOT = ROOT
zenodo.RELEASE = ROOT / "release" / "unit-12"
zenodo.TOKEN_FILE = Path.home() / "Documents" / "Obsidian notes" / "New zenodo token.md"
zenodo.RESERVATION = ROOT / "qa" / "UNIT_12_ZENODO_RESERVATION.json"
zenodo.RECEIPT = ROOT / "qa" / "UNIT_12_ZENODO_PUBLICATION.json"
...
zenodo.VERSION = "unit-12"
zenodo.FILES = [
    "kurva-aljabar-id-unit-12.pdf",
    "kurva-aljabar-id-unit-12.html",
    "kurva-aljabar-id-unit-12-source.zip",
    "kurva-aljabar-id-unit-12-authority-witnesses.zip",
    "BUILD_RECEIPT-unit-12.json",
    "LICENSE-unit-12.md",
    "ZENODO_FILE_MANIFEST-unit-12.json",
...
def validate_release_contract() -> dict:
    ...
    require_equal(actual_names, set(zenodo.FILES), "Unit 12 release inventory")
    if len(entries) != len(zenodo.FILES) or len(set(zenodo.FILES)) != 8:
        raise RuntimeError("Unit 12 release must contain exactly eight unique files")
    ...
    source_zip = validate_zip(
        ...
        {
            f"{prefix}source/id-ID/lecture-12.md",
            f"{prefix}source/id-ID/worksheet-12.md",
            f"{prefix}source/id-ID/worksheet-12-solutions.md",
            f"{prefix}source/id-ID/media-credits-unit-12.md",
            f"{prefix}backend/units-01-12/MANIFEST.json",
            f"{prefix}backend/units-01-12/records.jsonl",
            f"{prefix}backend/common-backend-v1-contract/upstream/"
            "backend-migration-receipt-v1.v0.42.0.schema.json",
            f"{prefix}qa/UNITS_01_12_MACHINE_QA.json",
            f"{prefix}qa/UNITS_01_12_BACKEND_QA.json",
            f"{prefix}qa/UNIT_12_RELEASE_CANDIDATE.json",
            f"{prefix}LICENSE.md",
        },
        "source ZIP",
    )
    authority_required = {
        f"{prefix}authority/wikiversity/unit-12/UNIT_AUTHORITY_MANIFEST.json",
...
def current_boundary_contract() -> dict:
    ...
    machine = load_json(ROOT / "qa" / "UNITS_01_12_MACHINE_QA.json", "machine QA")
    backend = load_json(
        ROOT / "backend" / "units-01-12" / "MANIFEST.json",
...
def metadata(contract: dict | None = None) -> dict:
    ...
    payload = {
        ...
        "description": (
            "<p><strong>Rilis kumulatif kerja Bahasa Indonesia (id-ID), Unit 1–12</strong>, "
            "dari <em>Algebraische Kurven (Osnabrück 2025–2026)</em> karya Holger Brenner. "
            "Checkpoint parsial ini memuat dua belas kuliah, dua belas lembar kerja dengan "
            "330 soal, seluruh 55 solusi publik yang tersedia pada revisi sumber yang "
            "dibekukan, dan 65 posisi media pembaca yang dilengkapi kredit. Ini belum "
            ...
            "(id-ID) edition, Units 1–12, of Holger Brenner's <em>Algebraische Kurven "
            "(Osnabrück 2025–2026)</em>, with 330 exercises, all 55 frozen public-source "
            f"solutions, 65 credited reader-media positions, self-contained HTML, a "
...
def validate_metadata(payload: dict, contract: dict) -> None:
    ...
    if missing:
        raise RuntimeError(f"Required Unit 12 description markers are missing: {missing}")
...
def public_latest() -> dict:
    ...
    if public_version == VERSION:
        require_equal(public_metadata.get("title"), TITLE, "existing Unit 12 public title")
        if record_id == PREVIOUS_RECORD_ID:
            raise RuntimeError("Unit 12 cannot reuse the previous public record identity")
        return record
    if record_id != PREVIOUS_RECORD_ID:
        raise RuntimeError(
            "A different Zenodo version became latest after record 22070936; "
            "refusing to create Unit 12 from an unreviewed predecessor"
        )
...
def write_receipt(record: dict, verified: list[dict]) -> None:
    ...
    if int(record["id"]) == PREVIOUS_RECORD_ID:
        raise RuntimeError("Published Unit 12 record reused the predecessor record ID")
    require_equal(
...
def self_check() -> dict:
    ...
    require_equal(PREVIOUS_RECORD_ID, 22070936, "previous Zenodo record ID")
    require_equal(CONCEPT_DOI, "10.5281/zenodo.22059686", "Zenodo concept DOI")
    require_equal(RELEASE, ROOT / "release" / "unit-12", "Unit 12 release directory")
...
def publish() -> dict:
    ...
    if result.get("action") == "verified_existing_unit_07":
        result["action"] = "verified_existing_unit_12"
    return result
