#!/usr/bin/env python3
"""Fail-closed structural, mathematical-surface, and artifact QA through Unit 4."""
...
PDF = BUILD / "algebraic-geometry-bridge-id-units-01-04.pdf"
HTML = BUILD / "index.html"
RECEIPT = BUILD / "BUILD_RECEIPT.json"
OUT = ROOT / "qa" / "UNITS_01_04_MACHINE_QA.json"
EXPECTED_PDF_PAGES = 80

SOURCES = (
    SOURCE / "frontmatter-units-01-04.md",
...
EXPECTED = {
    ...
    SOURCE / "lecture-04.md": {"math": 169, "images": 7, "headers": 15},
    SOURCE / "worksheet-04.md": {"math": 115, "images": 2, "exercises": 30, "headers": 33},
    SOURCE / "worksheet-04-solutions.md": {"math": 102, "images": 0, "solutions": 6, "headers": 7},
}
...
def verify_unit4_authority() -> dict:
    path = ROOT / "authority" / "wikiversity" / "unit-04" / "UNIT_AUTHORITY_MANIFEST.json"
    manifest = json.loads(path.read_text(encoding="utf-8"))
    require(manifest["unit_number"] == 4, "Unit 4 manifest unit")
    require(manifest["lecture"]["revid"] == 1112250, "Unit 4 lecture revision")
    require(manifest["worksheet"]["revid"] == 1075377, "Unit 4 worksheet revision")
    require(manifest["lecture_transclusion_closure"]["requested_template_count"] == 84, "Unit 4 lecture template count")
    require(manifest["lecture_transclusion_closure"]["missing_page_count"] == 0, "Unit 4 lecture missing transclusion")
    require(manifest["worksheet_transclusion_closure"]["requested_template_count"] == 140, "Unit 4 worksheet template count")
    require(manifest["worksheet_transclusion_closure"]["missing_page_count"] == 0, "Unit 4 worksheet missing transclusion")
    for row in manifest["files"]:
        witness = path.parent / row["file"]
        require(witness.stat().st_size == row["bytes"] and digest(witness) == row["sha256"], f"Unit 4 authority replay {row['file']}")
    return {"manifest_sha256": digest(path), "file_count": len(manifest["files"]), "lecture_revid": 1112250, "worksheet_revid": 1075377}


def main() -> int:
    for path in (*SOURCES, CSS, HTML, PDF, RECEIPT):
    ...
    require("V(\\mathfrak p)=\\varnothing" in combined and "\\mathfrak p=(X^2+1)" in combined, "Unit 4 edition note")
    require("Soal 9.20" in combined and "Korolari 4.9" in combined, "Unit 4 source hints")
    css_text = CSS.read_text(encoding="utf-8")
    require('math[display="block"]' in css_text, "responsive display-math selector missing")
    require("overflow-x: auto" in css_text, "local display-math overflow rule missing")
    require("overflow-wrap: anywhere" in css_text, "long-link wrapping rule missing")
    ...
    require(len(ids) == len(set(ids)) == 219, f"stable ID closure: {len(ids)}")
    ...
    solution_summary = {
        ...
        "unit_04": verify_solution_map(4, SOURCE / "worksheet-04-solutions.md", 30, 6),
    }
    rights_summary = {
        ...
        "unit_04": verify_rights("RIGHTS-unit-04.csv", "ASSET_CLOSURE-unit-04.json", 9, 11),
    }
    authority_summary_3 = verify_unit3_authority()
    authority_summary_4 = verify_unit4_authority()

    html_bytes = HTML.read_bytes()
    soup = BeautifulSoup(html_bytes, "html.parser")
    require(soup.html is not None and soup.html.get("lang") == "id-ID", "HTML language")
    require(soup.title is not None and soup.title.get_text(strip=True) == "Kurva Aljabar - Unit 1-4", "HTML title")
    ...
    require(len(images) == 38, f"HTML image count {len(images)}")
    ...
    require(len(soup.find_all("math")) == 1453, f"HTML MathML count {len(soup.find_all('math'))}")
    ...
    require("Solusi Publik Lembar Kerja 4" in html_text, "Unit 4 solutions absent from HTML")
    require("Kredit media Unit 4" in html_text, "Unit 4 media credits absent from HTML")
    ...
    require(receipt["through_unit"] == 4, "build receipt scope")
    ...
    require(EXPECTED_PDF_PAGES > 0, "expected PDF page count is not frozen")
    require(len(reader.pages) == EXPECTED_PDF_PAGES, f"PDF pages {len(reader.pages)}")
    require(reader.metadata is not None and reader.metadata.title == "Kurva Aljabar - Unit 1-4", "PDF title metadata")
    ...
    result = {
        ...
        "authority": {"unit_03": authority_summary_3, "unit_04": authority_summary_4},
        "html": {
            ...
            "remote_images": 0,
            "responsive_css_sha256": digest(CSS),
            "display_math_local_horizontal_scroll": True,
            "long_link_wrapping": True,
        },
        "pdf": {
        ...
        "check_families": [
            ...
            "unit3_and_unit4_revision_authority_manifest_and_file_hash_replay",
