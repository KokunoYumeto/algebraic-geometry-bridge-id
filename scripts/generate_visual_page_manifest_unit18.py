#!/usr/bin/env python3
"""Bind all final Units 1-18 PDF raster pages to a replayable QA manifest."""
...
RENDER_DIR = ROOT / "tmp" / "pdfs" / "unit-18-release-render"
OUTPUT = ROOT / "qa" / "UNITS_01_18_VISUAL_PAGE_MANIFEST.json"
PDF = ROOT / "build" / "reader-id" / "algebraic-geometry-bridge-id-units-01-18.pdf"
EXPECTED_PDF_SHA256 = "ba62b61759a50925dcefa1a3a0153c8b597ee1386dd2033b610dae622e33ed99"
EXPECTED_PAGES = 320
...
def main() -> int:
    ...
    for number, path in enumerate(pages, start=1):
        ...
        rows.append({"page": number, "path": path.relative_to(ROOT).as_posix(), "bytes": path.stat().st_size, "sha256": sha256(path), "width_px": width, "height_px": height, "visible_ink_fraction_gray_lt_248": ink_fraction})

    payload = {
        "schema": "ag-bridge-visual-page-manifest-v1",
        "status": "PASS",
        "pdf": {"path": PDF.relative_to(ROOT).as_posix(), "bytes": PDF.stat().st_size, "sha256": EXPECTED_PDF_SHA256, "pages": EXPECTED_PAGES},
        "render": {
            "renderer": "pdftoppm",
            "resolution_dpi": 96,
            "page_count": len(rows),
