#!/usr/bin/env python3
"""Export the deterministic cumulative backend through reader Units 1--3.

The verified Units 1--2 JSONL is a compatibility baseline. Its 1,466 records
are carried forward with their canonical record bytes unchanged; Unit 3,
...
BASELINE = ROOT / "backend" / "units-01-02"
OUT = ROOT / "backend" / "units-01-03"
BUILD_RECEIPT_PATH = ROOT / "build" / "reader-id" / "BUILD_RECEIPT.json"
MACHINE_QA_PATH = ROOT / "qa" / "UNITS_01_03_MACHINE_QA.json"
VISUAL_QA_PATH = ROOT / "qa" / "UNITS_01_03_VISUAL_QA.json"
PROTECTED_QA_PATH = ROOT / "qa" / "UNIT_03_PROTECTED_SURFACES.json"
AUTHORITY_MANIFEST_PATH = ROOT / "authority" / "wikiversity" / "unit-03" / "UNIT_AUTHORITY_MANIFEST.json"
UNIT3_MAP_PATH = ROOT / "authority" / "wikiversity" / "unit-03" / "ORDERED_EXERCISE_MAP.json"
UNIT3_RIGHTS_PATH = ROOT / "authority" / "RIGHTS-unit-03.csv"
UNIT3_ASSET_CLOSURE_PATH = ROOT / "authority" / "ASSET_CLOSURE-unit-03.json"
CORRECTIONS_PATH = ROOT / "00_control" / "CORRECTIONS.csv"
TERMINOLOGY_PATH = ROOT / "00_control" / "TERMINOLOGY.csv"

BASELINE_MANIFEST_SHA256 = "3918c8fcd8bd8df7d4a5c65072b29701dc3a1a2103169152e5bb8d8beac6e5df"
BASELINE_RECORDS_SHA256 = "688a60e87b29cca35007b0a0904bb9507084e91059409269d8d43ab6d6723779"
BASELINE_SCHEMA_SHA256 = "04ddde6bd4647a263d22313fcb1a610af425b933902e649feffc0b1c852c8dad"
BASELINE_RECORD_COUNT = 1466
...
PREVIOUS_EDITION = "edition.algebraic-geometry-bridge-id.units-01-02.2026-08-22"
CUMULATIVE_EDITION = "edition.algebraic-geometry-bridge-id.units-01-03.2026-08-22"
...
NEW_SOURCE_FILES = [
    ROOT / "source" / "id-ID" / "frontmatter-units-01-03.md",
    ROOT / "source" / "id-ID" / "lecture-03.md",
    ROOT / "source" / "id-ID" / "worksheet-03.md",
    ROOT / "source" / "id-ID" / "worksheet-03-solutions.md",
    ROOT / "source" / "id-ID" / "media-credits-unit-03.md",
]
...
NEW_CONCEPTS = {
    "AGT-0019": ("concept.zariski-topology", "Zariski topology"),
    "AGT-0020": ("concept.vanishing-ideal", "vanishing ideal"),
    "AGT-0021": ("concept.radical-ideal", "radical ideal"),
    "AGT-0022": ("concept.nilpotent-element", "nilpotent element"),
    "AGT-0023": ("concept.reduced-ring", "reduced ring"),
    "AGT-0024": ("concept.zariski-closure", "Zariski closure"),
}
...
for path, expected in (
    ...
    actual = digest(path)
    if actual != expected:
        raise SystemExit(f"Frozen Units 1--2 baseline mismatch: {rel(path)} {actual} != {expected}")
...
if len(baseline_records) != BASELINE_RECORD_COUNT or len(baseline_raw_by_id) != BASELINE_RECORD_COUNT:
    raise SystemExit("Frozen Units 1--2 record-count or stable-ID closure mismatch")

build_receipt = json.loads(BUILD_RECEIPT_PATH.read_text(encoding="utf-8"))
if build_receipt.get("schema") != "ag-bridge-build-receipt-v2" or build_receipt.get("through_unit") != 3:
    raise SystemExit("Current reader build receipt is not the Units 1--3 authority")
timestamp = build_receipt["built_utc"]
authority_manifest = json.loads(AUTHORITY_MANIFEST_PATH.read_text(encoding="utf-8"))
unit3_map = json.loads(UNIT3_MAP_PATH.read_text(encoding="utf-8"))
if authority_manifest.get("unit_number") != 3:
    raise SystemExit("Unit 3 authority manifest has the wrong unit number")
if unit3_map.get("exercise_count") != 22 or unit3_map.get("solution_count") != 2:
    raise SystemExit("Unit 3 exercise/solution authority map mismatch")
...
add(
    make_record(
        ...
        provenance={
            "units_01_02_baseline_manifest_sha256": BASELINE_MANIFEST_SHA256,
            "unit_3_authority_manifest_sha256": digest(AUTHORITY_MANIFEST_PATH),
            "unit_3_exercise_map_sha256": digest(UNIT3_MAP_PATH),
        },
        payload={
...
if [row["term_id"] for row in new_terminology_rows] != list(NEW_CONCEPTS):
    raise SystemExit("Unit 3 terminology rows are missing or out of order")
...
unit3_solution_entries = {
    int(row["exercise_number"]): row
    for row in unit3_map["entries"]
    if row.get("has_public_solution")
}


def source_provenance(source_path: Path, metadata: dict[str, str], identifier: str) -> dict[str, Any]:
    editorial = source_path.name in {"frontmatter-units-01-03.md", "media-credits-unit-03.md"}
    result: dict[str, Any] = {
        "source_edition_id": None if editorial else BRENNER_EDITION,
        "unit_3_authority_manifest": {
            "path": rel(AUTHORITY_MANIFEST_PATH),
            "sha256": digest(AUTHORITY_MANIFEST_PATH),
        },
    }
    if source_path.name == "lecture-03.md":
        result["upstream"] = compact_upstream(authority_manifest["lecture"])
    elif source_path.name == "worksheet-03.md":
        result["upstream"] = compact_upstream(authority_manifest["worksheet"])
    elif source_path.name == "worksheet-03-solutions.md":
        match = re.search(r"-sol-(\d+)$", identifier)
        result["exercise_solution_map"] = {
            "path": rel(UNIT3_MAP_PATH),
            "sha256": digest(UNIT3_MAP_PATH),
            "worksheet_revid": unit3_map["worksheet"]["revid"],
        }
        result["upstream"] = (
            compact_upstream(unit3_solution_entries[int(match.group(1))])
            if match
            else compact_upstream(unit3_map["worksheet"])
        )
    else:
        result["upstream"] = {
            key: metadata[key]
...
unit3_rights_rows = read_csv(UNIT3_RIGHTS_PATH)
if len(unit3_rights_rows) != 4:
    raise SystemExit("Unit 3 rights closure does not contain four positions")
for row in unit3_rights_rows:
    ...
    add(
        make_record(
            ...
            parent_id=image_parent_by_path.get(row["local_path"], "br-ak-2025-2026-l03"),
...
for unit in all_new_units:
    ...
    if unit_type == "solution" and exercise_number is not None:
        provenance["exercise_solution_authority"] = {
            "map_path": rel(UNIT3_MAP_PATH),
            "map_sha256": digest(UNIT3_MAP_PATH),
            "upstream": compact_upstream(unit3_solution_entries[exercise_number]),
        }
    typed = make_record(
        unit_type,
...
qa_specs = [
    ("qa.units0103.machine", MACHINE_QA_PATH, "source_math_topology_build_accessibility", "status"),
    ("qa.units0103.visual", VISUAL_QA_PATH, "all_page_and_full_resolution_visual_layout", "result"),
    ("qa.unit03.protected", PROTECTED_QA_PATH, "authority_formula_exercise_solution_and_media_fidelity", "status"),
]
...
artifact_paths = [
    ...
    ROOT / "build" / "reader-id" / "algebraic-geometry-bridge-id-units-01-03.pdf",
    BUILD_RECEIPT_PATH,
    MACHINE_QA_PATH,
    VISUAL_QA_PATH,
    PROTECTED_QA_PATH,
    ...
    UNIT3_MAP_PATH,
    UNIT3_RIGHTS_PATH,
    UNIT3_ASSET_CLOSURE_PATH,
]
new_artifact_ids: list[str] = []
for number, artifact_path in enumerate(artifact_paths, start=1):
    artifact_id = f"artifact.units0103.{number:02d}.{artifact_path.name.casefold().replace('_', '-').replace('.', '-')}"
...
def add_relation(
    ...
    *,
    ...
    add(
        make_record(
            "relation",
            f"relation.units0103.{relation_counter:04d}",
...
for exercise_number in sorted(unit3_solution_entries):
    add_relation(
        "solves",
        f"br-ak-2025-2026-w03-sol-{exercise_number:02d}",
        f"br-ak-2025-2026-w03-ex-{exercise_number:02d}",
        source_locator=rel(UNIT3_MAP_PATH),
    )
for row in unit3_rights_rows:
    add_relation(
        "illustrates",
        row["asset_id"],
        image_parent_by_path.get(row["local_path"], "br-ak-2025-2026-l03"),
        source_locator=row["description_url"],
    )
...
for exercise_number in sorted(unit3_solution_entries):
    add_relation(
        "solves",
        f"solution.br-ak-2025-2026-w03-sol-{exercise_number:02d}",
        f"exercise.br-ak-2025-2026-w03-ex-{exercise_number:02d}",
        source_locator=rel(UNIT3_MAP_PATH),
...
schema["$id"] = "https://example.invalid/algebraic-geometry-bridge/backend-record-units-01-03-v1.schema.json"
schema["title"] = "Algebraic Geometry Bridge cumulative backend record through Units 1--3"
...
source_binding_paths = [
    ...
    *NEW_SOURCE_FILES,
    ...
    TERMINOLOGY_PATH,
    Path(__file__),
    ROOT / "scripts" / "qa_backend_units_01_03.py",
]
manifest = {
    ...
    "units_01_02_baseline": {
        "manifest_path": "backend/units-01-02/MANIFEST.json",
        "manifest_sha256": BASELINE_MANIFEST_SHA256,
        "records_path": "backend/units-01-02/records.jsonl",
    ...
    "authority_bindings": {
        "unit_3_manifest": {"path": rel(AUTHORITY_MANIFEST_PATH), "sha256": digest(AUTHORITY_MANIFEST_PATH)},
        "unit_3_exercise_map": {"path": rel(UNIT3_MAP_PATH), "sha256": digest(UNIT3_MAP_PATH)},
        "unit_3_rights": {"path": rel(UNIT3_RIGHTS_PATH), "sha256": digest(UNIT3_RIGHTS_PATH)},
        "unit_3_asset_closure": {"path": rel(UNIT3_ASSET_CLOSURE_PATH), "sha256": digest(UNIT3_ASSET_CLOSURE_PATH)},
    },
    "validation": {
        ...
        "units_01_02_record_bytes_preserved": True,
    ...
    json.dumps(
        {
            ...
            "units_01_02_records_preserved": BASELINE_RECORD_COUNT,
