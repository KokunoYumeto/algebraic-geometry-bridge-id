#!/usr/bin/env python3
"""Fail-closed structural, mathematical-surface, and artifact QA through Unit 2."""
...
PDF = BUILD / "algebraic-geometry-bridge-id-units-01-02.pdf"
HTML = BUILD / "index.html"
RECEIPT = BUILD / "BUILD_RECEIPT.json"
OUT = ROOT / "qa" / "UNITS_01_02_MACHINE_QA.json"

SOURCES = (
    SOURCE / "frontmatter-units-01-02.md",
...
def solution_map(number: int) -> tuple[dict, Path]:
    if number == 1:
        ...
        path = ROOT / "authority" / "wikiversity" / "unit-02" / "ORDERED_EXERCISE_MAP.json"
...
def verify_solution_map(number: int, translated: Path, expected_exercises: int, expected_solutions: int) -> dict:
    ...
    if number == 1:
        ...
        require(len(entries) == expected_exercises, "Unit 2 map must enumerate solved and unsolved exercises")
...
def main() -> int:
    ...
    require(len(ids) == len(set(ids)) == 109, f"stable ID closure: {len(ids)}")
    ...
    require(soup.title is not None and soup.title.get_text(strip=True) == "Kurva Aljabar — Unit 1–2", "HTML title")
    ...
    require(len(images) == 25, f"HTML image count {len(images)}")
    ...
    require(len(soup.find_all("math")) == 798, f"HTML MathML count {len(soup.find_all('math'))}")
    require("Solusi Lembar Kerja 2" in soup.get_text(" ", strip=True), "Unit 2 solutions absent from HTML")
    require("Kredit media Unit 2" in soup.get_text(" ", strip=True), "Unit 2 media credits absent from HTML")
    ...
    require(receipt["through_unit"] == 2, "build receipt scope")
    ...
    require(len(reader.pages) == 48, f"PDF pages {len(reader.pages)}")
    require(reader.metadata is not None and reader.metadata.title == "Kurva Aljabar — Unit 1–2", "PDF title metadata")
    pdf_text = "\n".join(page.extract_text() or "" for page in reader.pages)
    for marker in ("Kuliah 2: Himpunan Aljabar Afin", "Soal 2.27", "Solusi Soal 2.20", "Kredit media Unit 2"):
    ...
    result = {
        "schema": "ag-bridge-machine-qa-receipt-v2",
        ...
        "check_families": [
            "source_placeholders_language_and_secrets",
