#!/usr/bin/env python3
"""Build the fail-closed, reader-first cumulative Unit 15 release payload."""
...
helpers.OUT = ROOT / "release" / "unit-15"
...
BACKEND_DIRECTORY = "backend/units-01-15"
...
PDF_NAME = "kurva-aljabar-id-unit-15.pdf"
HTML_NAME = "kurva-aljabar-id-unit-15.html"
SOURCE_ZIP_NAME = "kurva-aljabar-id-unit-15-source.zip"
AUTHORITY_ZIP_NAME = "kurva-aljabar-id-unit-15-authority-witnesses.zip"
BUILD_RECEIPT_NAME = "BUILD_RECEIPT-unit-15.json"
LICENSE_NAME = "LICENSE-unit-15.md"
MANIFEST_NAME = "ZENODO_FILE_MANIFEST-unit-15.json"
...
MACHINE_QA = "qa/UNITS_01_15_MACHINE_QA.json"
VISUAL_QA = "qa/UNITS_01_15_VISUAL_QA.json"
RESPONSIVE_QA = "qa/UNITS_01_15_RESPONSIVE_QA.json"
BACKEND_QA = "qa/UNITS_01_15_BACKEND_QA.json"
PROTECTED_QA = "qa/UNIT_15_PROTECTED_SURFACES.json"
RELEASE_CANDIDATE = "qa/UNIT_15_RELEASE_CANDIDATE.json"
HANDOFF = "qa/UNITS_01_15_HANDOFF.md"
...
def validate_release_candidate(pages: int, backend_records: int) -> dict:
    ...
    if not str(candidate.get("status", "")).startswith("PASS"):
        raise RuntimeError("Unit 15 release candidate has not passed")
...
def validate_license() -> Path:
    ...
    if missing:
        raise RuntimeError(f"Unit 15 mixed-rights licence notice is incomplete: {missing}")
    return path
...
def source_inventory() -> list[Path]:
    files: list[Path] = [
        ...
        exact(MACHINE_QA),
        exact(VISUAL_QA),
        exact("qa/UNITS_01_15_VISUAL_PAGE_MANIFEST.json"),
        exact(RESPONSIVE_QA),
        exact(BACKEND_QA),
        exact(PROTECTED_QA),
...
def verify_final_inventory() -> list[dict]:
    ...
    require_equal(set(by_name), set(RELEASE_FILES), "Unit 15 release inventory")
    if len(entries) != len(RELEASE_FILES):
        raise RuntimeError("Unit 15 release inventory contains duplicate or unexpected entries")
