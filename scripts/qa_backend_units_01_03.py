#!/usr/bin/env python3
"""Fail-closed replay and closure QA for the cumulative Units 1--3 backend."""
...
EXPORTER = ROOT / "scripts" / "export_backend_units_01_03.py"
BACKEND = ROOT / "backend" / "units-01-03"
MANIFEST = BACKEND / "MANIFEST.json"
RECEIPT = ROOT / "qa" / "UNITS_01_03_BACKEND_QA.json"
...
RIGHTS_PATH = ROOT / "authority" / "RIGHTS-unit-03.csv"
CLOSURE_PATH = ROOT / "authority" / "ASSET_CLOSURE-unit-03.json"
...
NEW_TERM_BINDINGS = {
    "AGT-0019": ("concept.zariski-topology", "topologi Zariski"),
    "AGT-0020": ("concept.vanishing-ideal", "ideal pelenyapan"),
    "AGT-0021": ("concept.radical-ideal", "ideal radikal"),
    "AGT-0022": ("concept.nilpotent-element", "unsur nilpoten"),
    "AGT-0023": ("concept.reduced-ring", "gelanggang tereduksi"),
    "AGT-0024": ("concept.zariski-closure", "penutupan Zariski"),
}
...
manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
require(manifest["through_unit"] == 3, "Manifest is not cumulative through Unit 3")
require(manifest["scope"] == "cumulative Units 1--3", "Manifest scope mismatch")
...
require(digest(BASELINE / "records.jsonl") == BASELINE_RECORDS_SHA256, "Units 1--2 baseline hash changed")
...
require(len(baseline_raw_by_id) == BASELINE_RECORD_COUNT, "Frozen Units 1--2 baseline record count changed")
for stable_id, baseline_line in baseline_raw_by_id.items():
    require(cumulative_raw_by_id.get(stable_id) == baseline_line, f"Units 1--2 record bytes changed: {stable_id}")
...
require(build_receipt["through_unit"] == 3, "Reader build receipt scope mismatch")
...
current_artifacts = {
    record["path"]: record
    for record in combined_records
    if record["entity_class"] == "artifact" and record["stable_id"].startswith("artifact.units0103.")
}
required_artifact_paths = [row["path"] for row in build_receipt["outputs"]] + [
    BUILD_RECEIPT_PATH.relative_to(ROOT).as_posix(),
    "qa/UNITS_01_03_MACHINE_QA.json",
    "qa/UNITS_01_03_VISUAL_QA.json",
    "qa/UNIT_03_PROTECTED_SURFACES.json",
]
...
require(authority_manifest["unit_number"] == 3, "Authority manifest unit mismatch")
authority_dir = AUTHORITY_MANIFEST_PATH.parent
for row in authority_manifest["files"]:
    path = authority_dir / row["file"]
    require(path.is_file(), f"Unit 3 authority file absent: {row['file']}")
    require(path.stat().st_size == row["bytes"], f"Unit 3 authority byte mismatch: {row['file']}")
    require(digest(path) == row["sha256"], f"Unit 3 authority hash mismatch: {row['file']}")
...
for source_name, authority_key in (("lecture-03.md", "lecture"), ("worksheet-03.md", "worksheet")):
...
worksheet_path = ROOT / "source" / "id-ID" / "worksheet-03.md"
solutions_path = ROOT / "source" / "id-ID" / "worksheet-03-solutions.md"
exercises = heading_ids(worksheet_path, r"br-ak-2025-2026-w03-ex-\d{2}")
solutions = heading_ids(solutions_path, r"br-ak-2025-2026-w03-sol-\d{2}")
require(len(exercises) == 22, "Unit 3 exercise heading count mismatch")
require(len(solutions) == 2, "Unit 3 solution heading count mismatch")
authority_map = json.loads(UNIT3_MAP_PATH.read_text(encoding="utf-8"))
...
require(source_solution_numbers == mapped_numbers == [11, 13], "Unit 3 solution-map mismatch")
require({f"exercise.{identifier}" for identifier in exercises} <= class_ids["exercise"], "Unit 3 exercise projections missing")
require({f"solution.{identifier}" for identifier in solutions} <= class_ids["solution"], "Unit 3 solution projections missing")
...
for number, authority_row in mapped_rows.items():
    solution_id = f"solution.br-ak-2025-2026-w03-sol-{number:02d}"
    exercise_id = f"exercise.br-ak-2025-2026-w03-ex-{number:02d}"
    ...
    require(binding["map_sha256"] == digest(UNIT3_MAP_PATH), f"Solution map hash absent: {solution_id}")
...
require(len(class_ids["exercise"]) == 77, "Cumulative exercise-family count mismatch")
require(len(class_ids["solution"]) == 18, "Cumulative solution-family count mismatch")

rights_rows = read_csv(RIGHTS_PATH)
closure = json.loads(CLOSURE_PATH.read_text(encoding="utf-8"))
require(len(rights_rows) == 4, "Unit 3 rights-row count mismatch")
require(closure["reader_media_positions"] == 4, "Unit 3 media-position mismatch")
require(closure["rights_sha256"] == digest(RIGHTS_PATH), "Unit 3 rights closure hash mismatch")
...
require(len(unique_local_paths) == closure["unique_local_assets"] == 4, "Unit 3 unique-asset closure mismatch")
require(len(class_ids["asset"]) == 29, "Cumulative primary-asset count mismatch")
...
require(len(class_ids["concept"]) == 24 and len(class_ids["term"]) == 24, "Cumulative terminology-family count mismatch")
...
result = {
    ...
    "units_01_02_record_bytes_preserved": len(baseline_raw_by_id),
    "parent_closure": True,
    ...
    "authority_binding": {
        "unit_3_manifest_sha256": digest(AUTHORITY_MANIFEST_PATH),
    ...
    "terminology": {
        ...
        "unit_03_rows": sorted(NEW_TERM_BINDINGS),
    ...
    "check_families": [
        ...
        "stable_id_uniqueness_format_and_units0102_byte_preservation",
        ...
        "unit3_exercise_solution_family_and_map_closure",
        "unit3_component_rights_asset_and_hash_closure",
        "unit3_terminology_control_and_concept_term_closure",
        "seven_row_correction_ledger_hash_closure",
