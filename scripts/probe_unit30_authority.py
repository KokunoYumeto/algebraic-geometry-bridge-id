#!/usr/bin/env python3
"""Bounded primary-source discovery for official 2012 Unit 30 authority."""
...
OUT = ROOT / "authority" / "wikiversity" / "unit-30"
...
base.RIGHTS = ROOT / "authority" / "RIGHTS-unit-30.csv"
base.CLOSURE = ROOT / "authority" / "ASSET_CLOSURE-unit-30.json"
base.COMMONS_META = OUT / "local-pdf-file-metadata-api.json"
base.USER_AGENT = "O016-unit30-authority-probe/1.0 (bounded educational preservation)"
...
def closure_summary(root: dict, parsed: dict, closure: dict) -> dict:
    ...
    records.sort(key=lambda item: (item["title"], item["pageid"]))
    payload = json.dumps(
        records,
        ensure_ascii=False,
        sort_keys=True,
...
def probe_main() -> dict:
    ...
    lecture, lecture_parsed = base.entry_surface(LECTURE_TITLE, "lecture-30")
    worksheet, worksheet_parsed = base.entry_surface(WORKSHEET_TITLE, "worksheet-30")
    lecture_latex, lecture_tex = base.latex_surface(
        LECTURE_TITLE + "/latex", "lecture-30"
    )
    worksheet_latex, worksheet_tex = base.latex_surface(
        WORKSHEET_TITLE + "/latex", "worksheet-30"
    )
    lecture_closure = base.transclusion_closure(lecture_parsed, "lecture-30")
    worksheet_closure = base.transclusion_closure(worksheet_parsed, "worksheet-30")
...
def probe_media() -> dict:
    lecture_parsed = json.loads(
        (OUT / "lecture-30-parse-api.json").read_text(encoding="utf-8")
    )["parse"]
    worksheet_parsed = json.loads(
        (OUT / "worksheet-30-parse-api.json").read_text(encoding="utf-8")
    )["parse"]
...
def probe_solution(number: int, title: str) -> dict:
    solution, parsed = base.entry_surface(title, f"solution-ex{number:02d}")
...
def main() -> int:
    ...
    if args.media_only:
        ...
        for entry in summary["solutions"]["entries"]:
            ...
            base.write_json(
                OUT / f"PROBE_SOLUTION_EX{entry['exercise_number']:02d}_SUMMARY.json",
                item,
            )
        ...
        output = OUT / f"PROBE_SOLUTION_EX{args.solution_number:02d}_SUMMARY.json"
    else:
        result = probe_main()
