#!/usr/bin/env python3
"""Export the deterministic cumulative backend through reader Units 1--4.

The verified Units 1--3 JSONL is a compatibility baseline. Its 2,003 records
are carried forward with their canonical record bytes unchanged; Unit 4,
...
OUT = ROOT / "backend" / "units-01-04"
BUILD_RECEIPT_PATH = ROOT / "build" / "reader-id" / "BUILD_RECEIPT.json"
MACHINE_QA_PATH = ROOT / "qa" / "UNITS_01_04_MACHINE_QA.json"
VISUAL_QA_PATH = ROOT / "qa" / "UNITS_01_04_VISUAL_QA.json"
RESPONSIVE_QA_PATH = ROOT / "qa" / "UNITS_01_04_RESPONSIVE_QA.json"
PROTECTED_QA_PATH = ROOT / "qa" / "UNIT_04_PROTECTED_SURFACES.json"
...
UNIT4_RIGHTS_PATH = ROOT / "authority" / "RIGHTS-unit-04.csv"
UNIT4_ASSET_CLOSURE_PATH = ROOT / "authority" / "ASSET_CLOSURE-unit-04.json"
CORRECTIONS_PATH = ROOT / "00_control" / "CORRECTIONS.csv"
TERMINOLOGY_PATH = ROOT / "00_control" / "TERMINOLOGY.csv"

BASELINE_MANIFEST_SHA256 = "e5b6d7387d39f3fd542a6b8519c0d141d966bbbd9f9b285dacfa78b8d2ca9b0b"
BASELINE_RECORDS_SHA256 = "5a54ed0b813145b57d3e81db41bc36c01cb94f9c39c7711f086117a09bb85a89"
BASELINE_SCHEMA_SHA256 = "803f90c89d8f5fdfce27e5ff2cdb355978aa1cdca000237a636c8d6fc7f10ff7"
...
PREVIOUS_EDITION = "edition.algebraic-geometry-bridge-id.units-01-03.2026-08-22"
...
NEW_SOURCE_FILES = [
    ROOT / "source" / "id-ID" / "frontmatter-units-01-04.md",
    ROOT / "source" / "id-ID" / "lecture-04.md",
    ROOT / "source" / "id-ID" / "worksheet-04.md",
    ROOT / "source" / "id-ID" / "worksheet-04-solutions.md",
    ROOT / "source" / "id-ID" / "media-credits-unit-04.md",
]
...
NEW_CONCEPTS = {
    "AGT-0025": ("concept.irreducible-component", "irreducible component"),
    "AGT-0026": ("concept.prime-ideal", "prime ideal"),
    "AGT-0027": ("concept.fraction-field", "field of fractions"),
    "AGT-0028": ("concept.rational-function-field", "rational function field"),
    "AGT-0029": ("concept.integral-domain", "integral domain"),
    "AGT-0030": ("concept.quotient-ring", "quotient ring"),
    "AGT-0031": ("concept.unique-factorization-domain", "unique factorization domain"),
    "AGT-0032": ("concept.principal-ideal-domain", "principal ideal domain"),
    "AGT-0033": ("concept.closed-map", "closed map"),
    "AGT-0034": ("concept.open-map", "open map"),
    "AGT-0035": ("concept.connected-space", "connected space"),
}
...
for path, expected in (
    ...
    actual = digest(path)
    if actual != expected:
        raise SystemExit(f"Frozen Units 1--3 baseline mismatch: {rel(path)} {actual} != {expected}")
...
if len(baseline_records) != BASELINE_RECORD_COUNT or len(baseline_raw_by_id) != BASELINE_RECORD_COUNT:
    raise SystemExit("Frozen Units 1--3 record-count or stable-ID closure mismatch")

build_receipt = json.loads(BUILD_RECEIPT_PATH.read_text(encoding="utf-8"))
if build_receipt.get("schema") != "ag-bridge-build-receipt-v2" or build_receipt.get("through_unit") != 4:
    raise SystemExit("Current reader build receipt is not the Units 1--4 authority")
timestamp = build_receipt["built_utc"]
authority_manifest = json.loads(AUTHORITY_MANIFEST_PATH.read_text(encoding="utf-8"))
unit4_map = json.loads(UNIT4_MAP_PATH.read_text(encoding="utf-8"))
if authority_manifest.get("unit_number") != 4:
    raise SystemExit("Unit 4 authority manifest has the wrong unit number")
if unit4_map.get("exercise_count") != 30 or unit4_map.get("solution_count") != 6:
    raise SystemExit("Unit 4 exercise/solution authority map mismatch")
...
add(
    make_record(
        ...
        provenance={
            "units_01_03_baseline_manifest_sha256": BASELINE_MANIFEST_SHA256,
            "unit_4_authority_manifest_sha256": digest(AUTHORITY_MANIFEST_PATH),
            "unit_4_exercise_map_sha256": digest(UNIT4_MAP_PATH),
        },
        payload={
...
if [row["term_id"] for row in new_terminology_rows] != list(NEW_CONCEPTS):
    raise SystemExit("Unit 4 terminology rows are missing or out of order")
...
new_correction_rows = [
    ...
    if row["correction_id"] in {"AGC-CORR-0005", "AGC-CORR-0006"}
]
if [row["correction_id"] for row in new_correction_rows] != ["AGC-CORR-0005", "AGC-CORR-0006"]:
    raise SystemExit("Unit 4 correction rows are missing or out of order")
correction_targets = {
    "AGC-CORR-0005": ["br-ak-2025-2026-w04-ex-06"],
    "AGC-CORR-0006": ["br-ak-2025-2026-l04-ex-03"],
}
...
unit4_solution_entries = {
    int(row["exercise_number"]): row
    for row in unit4_map["entries"]
    if row.get("has_public_solution")
}


def source_provenance(source_path: Path, metadata: dict[str, str], identifier: str) -> dict[str, Any]:
    editorial = source_path.name in {"frontmatter-units-01-04.md", "media-credits-unit-04.md"}
    result: dict[str, Any] = {
        "source_edition_id": None if editorial else BRENNER_EDITION,
        "unit_4_authority_manifest": {
            "path": rel(AUTHORITY_MANIFEST_PATH),
            "sha256": digest(AUTHORITY_MANIFEST_PATH),
        },
    }
    if source_path.name == "lecture-04.md":
        result["upstream"] = compact_upstream(authority_manifest["lecture"])
    elif source_path.name == "worksheet-04.md":
        result["upstream"] = compact_upstream(authority_manifest["worksheet"])
    elif source_path.name == "worksheet-04-solutions.md":
        match = re.search(r"-sol-(\d+)$", identifier)
        result["exercise_solution_map"] = {
            "path": rel(UNIT4_MAP_PATH),
            "sha256": digest(UNIT4_MAP_PATH),
            "worksheet_revid": unit4_map["worksheet"]["revid"],
        }
        result["upstream"] = (
            compact_upstream(unit4_solution_entries[int(match.group(1))])
            if match
            else compact_upstream(unit4_map["worksheet"])
        )
    else:
        result["upstream"] = {
            key: metadata[key]
...
unit4_rights_rows = read_csv(UNIT4_RIGHTS_PATH)
if len(unit4_rights_rows) != 9:
    raise SystemExit("Unit 4 rights closure does not contain nine positions")
for row in unit4_rights_rows:
    ...
    add(
        make_record(
            ...
            parent_id=image_parent_by_path.get(row["local_path"], "br-ak-2025-2026-l04"),
...
for unit in all_new_units:
    ...
    if unit_type == "solution" and exercise_number is not None:
        provenance["exercise_solution_authority"] = {
            "map_path": rel(UNIT4_MAP_PATH),
            "map_sha256": digest(UNIT4_MAP_PATH),
            "upstream": compact_upstream(unit4_solution_entries[exercise_number]),
        }
    typed = make_record(
        unit_type,
...
qa_specs = [
    ("qa.units0104.machine", MACHINE_QA_PATH, "source_math_topology_build_accessibility", "status"),
    ("qa.units0104.visual", VISUAL_QA_PATH, "all_page_and_full_resolution_visual_layout", "result"),
    ("qa.units0104.responsive", RESPONSIVE_QA_PATH, "desktop_and_mobile_reader_reflow", "status"),
    ("qa.unit04.protected", PROTECTED_QA_PATH, "authority_formula_exercise_solution_and_media_fidelity", "status"),
]
...
artifact_paths = [
    ...
    ROOT / "build" / "reader-id" / "algebraic-geometry-bridge-id-units-01-04.pdf",
    BUILD_RECEIPT_PATH,
    MACHINE_QA_PATH,
    VISUAL_QA_PATH,
    RESPONSIVE_QA_PATH,
    ...
    UNIT4_MAP_PATH,
    UNIT4_RIGHTS_PATH,
    UNIT4_ASSET_CLOSURE_PATH,
]
new_artifact_ids: list[str] = []
for number, artifact_path in enumerate(artifact_paths, start=1):
    artifact_id = f"artifact.units0104.{number:02d}.{artifact_path.name.casefold().replace('_', '-').replace('.', '-')}"
...
def add_relation(
    ...
    *,
    ...
    add(
        make_record(
            "relation",
            f"relation.units0104.{relation_counter:04d}",
...
for exercise_number in sorted(unit4_solution_entries):
    add_relation(
        "solves",
        f"br-ak-2025-2026-w04-sol-{exercise_number:02d}",
        f"br-ak-2025-2026-w04-ex-{exercise_number:02d}",
        source_locator=rel(UNIT4_MAP_PATH),
    )
for row in unit4_rights_rows:
    add_relation(
        "illustrates",
        row["asset_id"],
        image_parent_by_path.get(row["local_path"], "br-ak-2025-2026-l04"),
        source_locator=row["description_url"],
    )
...
for exercise_number in sorted(unit4_solution_entries):
    add_relation(
        "solves",
        f"solution.br-ak-2025-2026-w04-sol-{exercise_number:02d}",
        f"exercise.br-ak-2025-2026-w04-ex-{exercise_number:02d}",
        source_locator=rel(UNIT4_MAP_PATH),
...
schema["$id"] = "https://example.invalid/algebraic-geometry-bridge/backend-record-units-01-04-v1.schema.json"
schema["title"] = "Algebraic Geometry Bridge cumulative backend record through Units 1--4"
...
source_binding_paths = [
    ...
    *NEW_SOURCE_FILES,
    ...
    TERMINOLOGY_PATH,
    Path(__file__),
    ROOT / "scripts" / "qa_backend_units_01_04.py",
]
manifest = {
    ...
    "units_01_03_baseline": {
        "manifest_path": "backend/units-01-03/MANIFEST.json",
        "manifest_sha256": BASELINE_MANIFEST_SHA256,
        "records_path": "backend/units-01-03/records.jsonl",
    ...
    "authority_bindings": {
        "unit_4_manifest": {"path": rel(AUTHORITY_MANIFEST_PATH), "sha256": digest(AUTHORITY_MANIFEST_PATH)},
        "unit_4_exercise_map": {"path": rel(UNIT4_MAP_PATH), "sha256": digest(UNIT4_MAP_PATH)},
        "unit_4_rights": {"path": rel(UNIT4_RIGHTS_PATH), "sha256": digest(UNIT4_RIGHTS_PATH)},
        "unit_4_asset_closure": {"path": rel(UNIT4_ASSET_CLOSURE_PATH), "sha256": digest(UNIT4_ASSET_CLOSURE_PATH)},
    },
    "validation": {
        ...
        "units_01_03_record_bytes_preserved": True,
    ...
    json.dumps(
        {
            ...
            "units_01_03_records_preserved": BASELINE_RECORD_COUNT,
