...
Unit 24–28 mengikuti kuliah dan lembar kerja resmi *Osnabrück 2012*. Kedua
...
Status saat ini: **edisi parsial 28 dari 30 unit; checkpoint kumulatif Unit
1–28 lengkap untuk batas ini dan terverifikasi**. Checkpoint ini memuat 28
kuliah, 28 lembar kerja dengan seluruh 671 soal, semua 118 solusi publik yang
tersedia pada revisi sumber yang dibekukan, 98 posisi media beserta atribusi
per komponen, HTML mandiri, PDF A4 476 halaman, dan backend ID stabil dengan
21.358 rekaman. Dua unit klasik masih tersisa; Unit 29 adalah batas produksi
berikutnya.
...
- [Checkpoint Unit 28 di Zenodo](https://doi.org/10.5281/zenodo.22105836)
...
- `build/reader-id/algebraic-geometry-bridge-id-units-01-28.pdf`: PDF A4 kumulatif
...
judul semantik, teks alternatif pada semua 98 gambar, 10.717 simpul MathML,
1.520 tautan internal yang lengkap, dan reflow seluler tanpa luapan halaman.
...
- `authority/wikiversity/unit-01` sampai `unit-28` mengikat revisi kuliah dan
...
- `authority/RIGHTS.csv` dan `authority/RIGHTS-unit-02.csv` sampai
  `authority/RIGHTS-unit-28.csv` memisahkan hak setiap posisi media dan aset.
- [`build/reader-id/BUILD_RECEIPT.json`](build/reader-id/BUILD_RECEIPT.json)
  mengikat seluruh masukan dan keluaran pembaca.
- [`qa/UNITS_01_28_MACHINE_QA.json`](qa/UNITS_01_28_MACHINE_QA.json),
  [`qa/UNITS_01_28_VISUAL_QA.json`](qa/UNITS_01_28_VISUAL_QA.json),
  [`qa/UNITS_01_28_VISUAL_PAGE_MANIFEST.json`](qa/UNITS_01_28_VISUAL_PAGE_MANIFEST.json),
  [`qa/UNITS_01_28_RESPONSIVE_QA.json`](qa/UNITS_01_28_RESPONSIVE_QA.json), dan
  [`qa/UNIT_28_PROTECTED_SURFACES.json`](qa/UNIT_28_PROTECTED_SURFACES.json)
  merekam gerbang pembaca, semua 476 raster halaman, reflow desktop/seluler,
  matematika, struktur soal-solusi, hak, serta koreksi terlacak.
- [`backend/units-01-28/MANIFEST.json`](backend/units-01-28/MANIFEST.json)
  mengikat 21.358 rekaman deterministik; seluruh 20.570 rekaman baseline Unit
  1–27 dipertahankan byte demi byte.
- [`qa/UNITS_01_28_BACKEND_QA.json`](qa/UNITS_01_28_BACKEND_QA.json) mengikat
...
```powershell
python scripts/build_reader.py --through 28
python scripts/qa_reader_units_01_28.py
python scripts/qa_protected_surfaces_unit_28.py
python scripts/export_backend_units_01_28.py
python scripts/qa_backend_units_01_28.py
python scripts/generate_common_backend_v1_receipts.py --native-backend backend/units-01-28 --preflight
```

Gerbang visual merasterkan seluruh 476 halaman, mengikat setiap PNG menurut
...
px, 98/98 gambar, 10.717 simpul MathML, 1.520 tautan internal, 170 blok
...
24–28 mempertahankan pemberitahuan komponen jalur kursus CC BY-SA 4.0 dan
